...
export default async function VisitorDetailPage({ params }: Props) {
  ...
  const visitorSession = await prisma.visitorSession.findUnique({
    where: { id: sessionId },
    include: { pageViews: { orderBy: { createdAt: "asc" } } },
  });
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--ca-text3)", marginTop: 4 }}>
            {visitorSession.browser} · {visitorSession.os} · {visitorSession.device} · {visitorSession.country || "unknown country"} · {fmtDate(visitorSession.firstSeenAt)}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 16, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <Card>
            <CardHeader
              title={`Page journey — ${visitorSession.pageViews.length} page${visitorSession.pageViews.length !== 1 ? "s" : ""}`}
              right={<span style={{ fontSize: 11, color: "var(--ca-text3)" }}>Total: {fmtDuration(totalDuration)}</span>}
            />
            {visitorSession.pageViews.length ? (
              <div>
                {/* Table header */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 90px 100px", padding: "7px 14px", borderBottom: "1px solid var(--ca-bd)", gap: 8 }}>
                  {["Page", "Time spent", "Visited at"].map(h => (
                    <span key={h} style={{ fontSize: 10, fontWeight: 700, color: "var(--ca-text3)", textTransform: "uppercase", letterSpacing: "0.08em" }}>{h}</span>
                  ))}
                </div>
                {visitorSession.pageViews.map((pv: typeof visitorSession.pageViews[0], i: number) => (
                  <div key={pv.id} style={{ display: "grid", gridTemplateColumns: "1fr 90px 100px", padding: "9px 14px", gap: 8, borderTop: i > 0 ? "1px solid var(--ca-bd)" : "none", alignItems: "start" }}>
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        <span style={{ fontSize: 12.5, fontWeight: 500, color: "var(--ca-text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={pv.page}>{pv.page}</span>
                      </div>
                      {pv.title && (
                    ...
                    <div>
                      <span style={{
                        ...
                        color: !pv.duration ? "var(--ca-text3)" : pv.duration >= 60 ? "#22C55E" : pv.duration >= 15 ? "#F59E0B" : "var(--ca-text2)",
                      }}>
                      ...
                      {pv.duration && pv.duration > 0 && (
                        <div style={{ height: 2, borderRadius: 99, background: "var(--ca-raise)", marginTop: 4, overflow: "hidden" }}>
                          <div style={{ height: "100%", borderRadius: 99, background: pv.duration >= 60 ? "#22C55E" : pv.duration >= 15 ? "#F59E0B" : "#6B7280", width: `${Math.min(100, Math.round((pv.duration / Math.max(...visitorSession.pageViews.map((p: typeof visitorSession.pageViews[0]) => p.duration ?? 0), 1)) * 100))}%` }} />
                        </div>
                      )}
                    </div>
                    <span style={{ fontSize: 11, color: "var(--ca-text3)" }}>
                      {new Intl.DateTimeFormat("en-US", { hour: "numeric", minute: "2-digit", second: "2-digit" }).format(new Date(pv.createdAt))}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <Card style={{ padding: 14 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[
                ...
                { label: "Avg / page",    value: visitorSession.pageViews.length ? fmtDuration(Math.round(totalDuration / visitorSession.pageViews.length)) : "—", color: "#F59E0B" },
              ].map(s => (
