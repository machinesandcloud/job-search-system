...
import { Layout, CtaButton, Blockquote, Divider, Signature, Step, StatRow, p, muted, SITE_URL as APP } from "../base";
...
export function WinBack60({ firstName, unsubscribeUrl }: { firstName?: string; unsubscribeUrl: string }) {
  return (
    <Layout preview="Two months out. Here's what Zari users are doing right now." headline="Two months out. Still searching?" unsubscribeUrl={unsubscribeUrl}>
      ...
      <Text style={p()}>
        The job market rewards specificity and relentlessness. If there's a specific bottleneck (applications, interviews, salary negotiation), Zari can help you identify and fix it.
      </Text>
      <Text style={p()}>
