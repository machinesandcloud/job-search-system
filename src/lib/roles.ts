...
const genericFocus = "clear positioning, measurable impact, and execution clarity";

const makeGenericRole = (title: string): RoleProfile => {
  ...
  return {
    ...
    title,
    focus: genericFocus,
    keywords: Array.from(new Set([...words, "Impact", "Execution", "Scope", "Metrics"])),
    proofStrategy: [
      "Share a concise impact story tied to outcomes.",
      "Publish a short insight post that shows domain expertise.",
      "Build a proof asset (doc, demo, or analysis) aligned to the role.",
    ],
    interviewTopics: ["Impact stories", "Problem solving", "Cross-functional execution", "Leadership"],
    scripts: {
      referral: `Hey {firstName}, I'm exploring ${title} roles and would love a quick perspective or intro if possible.`,
      recruiter: `Hi {firstName} - I'm targeting ${title} roles and recently delivered measurable outcomes in scope and execution. Open to roles where impact is the core priority.`,
      followup: [
        `Following up with a short impact snapshot aligned to ${title} work.`,
...
export const roles: RoleProfile[] = [
  ...baseRoles,
  ...extraRoleTitles.map((title) => makeGenericRole(title)),
];
